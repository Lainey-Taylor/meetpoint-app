...
function cuisineLabel(item: RestaurantResult) {
  const generic = new Set(['餐饮服务', '中餐厅', '外国餐厅', '餐厅', '综合酒楼', '特色/地方风味餐厅']);
  const labels = [item.tag, item.type].filter(Boolean).flatMap((text) => String(text).split(/[;|·]/)).map((text) => text.trim()).filter(Boolean);
  return labels.find((label) => !generic.has(label) && /菜|火锅|烧烤|烤肉|料理|小吃|甜品|咖啡|面馆|西餐|快餐/.test(label)) || labels.find((label) => !generic.has(label));
}
function RestaurantCard({ item, rank, relaxed }: { item: RestaurantResult; rank: number; relaxed: boolean }) {
  const cuisine = cuisineLabel(item);
  return <Card style={styles.restaurant}>
    <View style={styles.restHead}><View style={styles.rank}><Text style={styles.rankText}>{rank}</Text></View><View style={{ flex: 1 }}><View style={styles.restTitleRow}><Text style={styles.restName}>{item.name}</Text>{cuisine ? <Text style={styles.cuisineTag}>{cuisine}</Text> : null}</View><Text style={styles.restMeta}>{[item.district, item.address].filter(Boolean).join(' · ')}</Text></View><View><Text style={styles.rating}>★ {item.rating || '待定'}</Text><Text style={styles.cost}>{item.cost ? `¥${item.cost}/人` : '价格待确认'}</Text></View></View>
    ...
    <View style={styles.actions}><View style={{ flex: 1 }}><PrimaryButton label="查看餐厅与路线" icon="map.fill" /></View><Pressable style={styles.more}><AppIcon name="ellipsis" size={18} color={colors.muted} /></Pressable></View>
  </Card>;
}
...
const styles = StyleSheet.create({
  ...
  restaurant: { gap: 11 }, restHead: { flexDirection: 'row', alignItems: 'flex-start', gap: 9 }, rank: { width: 29, height: 29, borderRadius: 10, backgroundColor: colors.brand, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' }, rankText: { color: '#fff', textAlign: 'center', fontWeight: '900', fontSize: 12, lineHeight: 15 }, restTitleRow: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 6 }, restName: { color: colors.text, fontSize: 14, fontWeight: '900' }, cuisineTag: { color: '#9A421D', backgroundColor: colors.brandSoft, borderRadius: 6, overflow: 'hidden', paddingHorizontal: 6, paddingVertical: 3, fontSize: 8, fontWeight: '800' }, restMeta: { color: colors.muted, fontSize: 10, marginTop: 4 }, rating: { color: colors.warning, fontSize: 11, fontWeight: '900', textAlign: 'right' }, cost: { color: colors.muted, fontSize: 9, textAlign: 'right', marginTop: 4 }, overrun: { color: colors.danger, backgroundColor: colors.dangerSoft, paddingHorizontal: 9, paddingVertical: 6, borderRadius: 9, overflow: 'hidden', fontSize: 10, fontWeight: '700' },
  people: { flexDirection: 'row', gap: 7 }, person: { flex: 1, minWidth: 94, backgroundColor: '#F5F8F7', borderRadius: 12, padding: 8, alignItems: 'center' }, personBad: { backgroundColor: colors.dangerSoft }, personName: { color: colors.brand, fontSize: 11, fontWeight: '800', maxWidth: '100%' }, personTime: { color: colors.text, fontSize: 14, fontWeight: '900', marginTop: 4 }, personMode: { color: colors.faint, fontSize: 8, marginTop: 2 }, actions: { flexDirection: 'row', gap: 8 }, more: { width: 48, borderRadius: 16, backgroundColor: colors.subtle, alignItems: 'center', justifyContent: 'center' },
});
