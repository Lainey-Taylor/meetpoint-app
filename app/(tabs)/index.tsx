...
export default function HomeScreen() {
  ...
  return (
    <Screen>
      <ImageBackground source={require('../../assets/images/stitch-hero.jpg')} resizeMode="cover" style={styles.hero}>
        ...
        <View style={styles.heroCopy}>
          ...
          <Pressable style={styles.directButton} onPress={() => router.push('/direct')}><AppIcon name="scope" size={17} color="#fff" /><Text style={styles.directText}>距离测算</Text><AppIcon name="arrow.right" size={16} color="#fff" /></Pressable>
        </View>
      </ImageBackground>

      <Pressable style={styles.intentCard} onPress={() => router.push('/choose')}>
        <View style={styles.intentIcon}><AppIcon name="plus" size={24} color={colors.brand} /></View>
      ...
      <View style={styles.filters}><View style={styles.filterActive}><Text style={styles.filterActiveText}>全部</Text></View>{['待出行', '进行中', '已结束'].map((item) => <View key={item} style={styles.filter}><Text style={styles.filterText}>{item}</Text></View>)}</View>

      <EventCard image={require('../../assets/images/stitch-event-upcoming.jpg')} date="本周六 · 18:30" title="第 3 次出行（三里屯太古里）" meta="★ 评分 4.9  ·  人均 ¥158  ·  4人已确认" status="待出行" onPress={continueRecentDinner} />
      <EventCard image={require('../../assets/images/stitch-event-past.jpg')} date="上周五 · 已圆满聚餐" title="第 2 次出行（望京万科时代）" meta="★ 评分 4.8  ·  人均 ¥125  ·  6人已确认" status="已结束" onPress={() => router.push('/results')} />
