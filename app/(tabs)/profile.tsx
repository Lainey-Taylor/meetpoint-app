    ...
    <Screen>
      ...
      <ImageBackground source={require('../../assets/images/profile-earth.jpg')} resizeMode="cover" style={styles.earthCard} imageStyle={styles.earthImage}>
        ...
        <View style={styles.earthContent}>
          ...
          <View style={styles.metrics}><Metric label="默认首选" value="公交地铁" /><Metric label="通勤上限" value="45 分" /><Metric label="组织聚餐" value="12 场" last /></View>
        </View>
      </ImageBackground>
...
function Metric({ label, value, last }: { label: string; value: string; last?: boolean }) { return <View style={[styles.metric, !last && styles.metricBorder]}><Text style={styles.metricLabel}>{label}</Text><Text style={styles.metricValue}>{value}</Text></View>; }
...
const styles = StyleSheet.create({
  ...
  earthCard: { height: 210, borderRadius: 23, overflow: 'hidden', justifyContent: 'flex-end' }, earthImage: { borderRadius: 23 }, earthShade: { position: 'absolute', inset: 0, backgroundColor: 'rgba(9,31,47,.20)' }, earthContent: { padding: 17 }, earthEyebrow: { color: '#fff', fontSize: 12, fontWeight: '800', textShadowColor: '#1238', textShadowRadius: 4 }, earthCountRow: { flexDirection: 'row', alignItems: 'baseline', gap: 7, marginTop: 2 }, earthCount: { color: '#fff', fontSize: 36, lineHeight: 43, fontWeight: '900', textShadowColor: '#1238', textShadowRadius: 5 }, earthUnit: { color: '#fff', fontSize: 13, fontWeight: '800' },
  metrics: { marginTop: 13, paddingVertical: 10, borderRadius: 14, backgroundColor: 'rgba(255,255,255,.88)', flexDirection: 'row' }, metric: { flex: 1, alignItems: 'center', paddingHorizontal: 5 }, metricBorder: { borderRightWidth: StyleSheet.hairlineWidth, borderRightColor: 'rgba(24,32,51,.22)' }, metricLabel: { color: colors.muted, fontSize: 9, fontWeight: '700' }, metricValue: { color: colors.text, fontSize: 12, fontWeight: '900', marginTop: 3 },
